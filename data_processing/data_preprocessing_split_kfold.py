...
if __name__ == '__main__':
    ...
    raw_df = pd.read_csv('./data_preprocessing_combine_output/combine_fc_kd_KeptGPT_foranova.csv')
    #''' for all features/ drop ALT/drop AST
    raw_df = raw_df.drop(columns = ['ID','性別'])
    
    #'''


    raw_df = raw_df.rename(columns = lambda x:re.sub('[^A-Za-z0-9_]+', 'p', x))
    split_kfold(raw_df = raw_df,random_state= 30,folderpath = './split_k_fold_output')
    '''==================================cancer======================================'''
    #raw_df = pd.read_csv('D:/CODE/cancer/data/ver1/combine_normal_stage.csv')

    # raw_df = raw_df.rename(columns = lambda x:re.sub('[^A-Za-z0-9_]+', 'p', x))
    # split_kfold(raw_df = raw_df,random_state= 30,folderpath = 'D:/CODE/cancer/data/ver1/k-fold/30')
